...
/* Returned by the NAME_RAW token in python.gram */
void *
_PyPegen_name_raw_token(Parser *p)
{
    return _PyPegen_expect_token(p, NAME);
}

const char* builtin_html_elements[] = {
    "a",
    "abbr",
    "address",
    "area",
...
/* Check if the component name is a built-in HTML element */
int // bool
_PyPegen_jsx_is_builtin_tag(Token *name) {
    const char *tag = PyBytes_AsString(name->bytes);
    const size_t len = sizeof(builtin_html_elements) / sizeof(builtin_html_elements[0]);
    for (size_t i = 0; i < len; i++) {
        const char *keyword = builtin_html_elements[i];
